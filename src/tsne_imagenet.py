...
parser.add_argument('--num-plots', default=5, type=int,
                    help='number of plots (each plot with a different seed)')
...
parser.add_argument('--num-classes', default=20, type=int,
                    help='number of classes to visualize (-1 for all)')
...
def main():
    ...
    # run inference
    targets, features = validate(val_loader, model, args)

    for seed in range(args.num_plots):
        targets_i = np.copy(targets)
        features_i = np.copy(features)

        # filter a subset of classes
        if args.num_classes != -1:
            np.random.seed(seed)
            subset_classes = np.random.choice(len(set(targets_i)), args.num_classes, replace=False)
            subset_indices = np.array([idx_x for idx_x, x in enumerate(targets_i) if x in subset_classes])
            targets_i = targets_i[subset_indices]
            features_i = features_i[subset_indices]

        # filter a subset of samples
        if args.num_samples != -1:
            np.random.seed(seed)
            subset_indices = np.random.choice(len(targets_i), args.num_samples, replace=False)
            targets_i = targets_i[subset_indices]
            features_i = features_i[subset_indices]

        target_ids = list(set(targets_i))

        # compute and visualize tsne
        tsne = TSNE(n_components=2, random_state=0, n_jobs=-1)

        print('=> run T-SNE..')
        features_2d = tsne.fit_transform(features_i)
        print(features_2d.shape)
        print('[Done]')

        print('=> plotting..')
        plt.figure(figsize=(6, 5))
        colors = plt.cm.get_cmap('hsv', args.num_classes)
        for idx, target in enumerate(target_ids):
            features_i_0 = features_2d[targets_i == target, 0]
            features_i_1 = features_2d[targets_i == target, 1]
            plt.scatter(features_i_0, features_i_1, c=colors(idx), s=10.0)
        ax = plt.gca()
        ax.axes.xaxis.set_visible(False)
        ax.axes.yaxis.set_visible(False)
        plt.savefig(os.path.join(args.save_path, 'tsne_imagenet_{}.pdf'.format(seed)), bbox_inches='tight')
        print('[Done]')

        # print labels of classes
        print('=> print names of classes..')
        class_idx_to_label = ClassIdxToLabel()
        for target_id in target_ids:
            print(class_idx_to_label.convert(target_id))
        print('[Done]')


def validate(val_loader, model, args):
    all_features = np.zeros((len(val_loader.dataset.targets), 2048), dtype=np.float)
    ...
    with torch.no_grad():
        ...
        for i, (images, targets) in enumerate(val_loader):
            ...
            # save features
            all_features[idx: idx + num_samples] = output
...
class ClassIdxToLabel(object):
    def __init__(self):
        self.idx_to_label = {0: 'tench, Tinca tinca',
